...
const LAYER_ORDER: Product['subCategory'][] = [
  PRODUCT_CATEGORIES.BIRD,
  PRODUCT_CATEGORIES.HAT,
  PRODUCT_CATEGORIES.SHOES,
  PRODUCT_CATEGORIES.BOTTOM,
  PRODUCT_CATEGORIES.TOP,
  PRODUCT_CATEGORIES.ACC,
  PRODUCT_CATEGORIES.EFFECT,
];
...
export const CharacterPreview = ({
  ...
  const selectedBird = previewItems.find(
    (item) => item.subCategory === PRODUCT_CATEGORIES.BIRD,
  );
  ...
  const wearables = useMemo(() => {
    return previewItems
      .filter(
        (item) =>
          item.subCategory !== PRODUCT_CATEGORIES.BIRD &&
          item.subCategory !== PRODUCT_CATEGORIES.ITEM,
      )
      .sort((a, b) => {
